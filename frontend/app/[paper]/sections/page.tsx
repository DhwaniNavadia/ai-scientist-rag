...
import { SectionsViewer } from "@/components/viewers/SectionsViewer";
import { getDownloadUrl } from "@/lib/api";

export default function SectionsPage({ params }: { params: Promise<{ paper: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-4 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-foreground">Paper Sections</h1>
          <p className="text-sm text-muted-foreground mt-0.5">
            Extracted structural sections from the PDF
          </p>
        </div>
        {data && !demoMode && (
          <DownloadButton
            label="Download JSON"
            href={getDownloadUrl(paper, "sections")}
          />
        )}
      </div>
      ...
      {data && (
        <SectionsViewer sections={data.sections} paperTitle={data.paper_title} />
      )}
    </div>
  );
}
